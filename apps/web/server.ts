...
async function main(): Promise<void> {
   ...
   */
  /**
   * The two calls an integrator makes.
   *
   * Deliberately separate from /api/redeem, which streams the demo's stages
   * back as SSE for a person watching. An integrator's server wants one JSON
   * answer, and a demo affordance is a bad thing to build a product on.
   *
   * The bearer token is required whenever one is configured. On loopback
   * there is nothing to protect against; behind a public URL there is, and
   * the check is the same either way so a deployment cannot forget it.
   */
  const authorised = (request: { headers: Record<string, unknown> }): boolean => {
    const expected = process.env.MAILPROOF_DAEMON_TOKEN?.trim();
    if (!expected) return true;
    const header = String(request.headers.authorization ?? '');
    const offered = header.startsWith('Bearer ') ? header.slice(7).trim() : '';
    // Length-independent comparison would be nicer, but the token is a
    // fixed-length hex string generated by us, so equality leaks nothing an
    // attacker does not already have to guess wholesale.
    return offered.length > 0 && offered === expected;
  };

  app.post('/api/verify/start', async (request, reply) => {
    if (!authorised(request as never)) {
      return reply.status(401).send({ error: 'missing or wrong bearer token' });
    }
    const secret = process.env.MAILPROOF_CHALLENGE_SECRET?.trim();
    const audience = process.env.MAILPROOF_AUDIENCE?.trim();
    if (!secret || !audience) {
      return reply.status(503).send({
        error: 'MAILPROOF_CHALLENGE_SECRET and MAILPROOF_AUDIENCE are not configured',
      });
    }
    ...
    const challenge = issueChallenge({
      ...
      audience,
      ttlMs: Number(process.env.MAILPROOF_CHALLENGE_TTL_MS ?? 15 * 60 * 1000),
    });
    return {
      code: challenge.code,
  ...
  app.post('/api/verify/finish', async (request, reply) => {
    if (!authorised(request as never)) {
      return reply.status(401).send({ error: 'missing or wrong bearer token' });
    }
    const eml = typeof request.body === 'string' ? request.body : '';
    if (eml.trim().length === 0) {
      return reply.status(400).send({ error: 'send the whole .eml as the request body' });
    }

    const entry = dkimDirectEntry();
    if (!entry) {
      return reply.status(503).send({
        error: `blueprint ${config.blueprintSlug} cannot verify a message on this deployment`,
      });
    }

    // Serialised with redemptions: this spends a claim and borrows the wallet,
    ...
    return redemptions.run(async () => {
      try {
        ...
        try {
          ...
          return {
            ...
            trust: {
              ...
              cryptographic: health?.cryptographicVerification ?? false,
              attestorKeyId: attestation.attestorKeyId,
            },
          };
        } catch (error) {
          // Already spent is the answer to "has this person had their
          // benefit", not a failure of the request.
        ...
        // A rejection is a verdict about the message, and the caller needs to
        // be able to tell the person which step failed.
        if (error instanceof AttestorRejection) {
